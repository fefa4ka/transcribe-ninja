...
class ExportTXT:
    def _to_txt(self, transcriptions, file_name):
        # file_name = md5(
        #     str(time.time())
        # ).hexdigest()
        ...
        with codecs.open(txt_file_path, 'w+', 'utf-8') as txt_file:
            ...
            for index, transcription in enumerate(transcriptions):
                ...
                part_file_path = os.path.join(settings.TEMP_DIR, "%s (%d).mp3.txt" % (file_name, index))
                with codecs.open(part_file_path, 'w+', 'utf-8') as part_file:
