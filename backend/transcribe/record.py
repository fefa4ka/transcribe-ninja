...
class Record(AudioFile, Trash):
    ...
    def save_to_pieces(self):
        from work.models import Queue
        import codecs


        order = self.orders.all()[0]
        queues = order.queue.filter(work_type=Queue.EDIT).order_by('piece__start_at')

        for index, queue in enumerate(queues):
            transcriptions = queue.transcriptions.all()

            piece_file_path = os.path.join(settings.TEMP_DIR, "%s (%d).mp3" % (self.title, index))

            self.cut_to_file(
                file_name=piece_file_path,
                start_at=queue.start_at,
                end_at=queue.end_at,
                offset=0
            )

            transcription_file_path = "%s.txt" % piece_file_path
            with codecs.open(transcription_file_path, 'w+', 'utf-8') as part_file:
                for transcription in transcriptions:
                    part_file.write(transcription.text_pretty)
